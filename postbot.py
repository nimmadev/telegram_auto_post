#!/usr/bin/env python
_Z='but bot👇\u200c\u200c'
_Y='User %s started the post.'
_X='User %s canceled the conversation.'
_W='below is the post number for your post\n\nuse below  command to send post to ur group \n\ntime is in seconds'
_V='skiping link as u entered incorrect link'
_U='Please enter link name for second button\n\n or press /done'
_T='send the photo 📸\n\n/skip the Photo'
_S='working'
_R='Send Link'
_Q='linkname'
_P='✅Done'
_O='t.me/r5pro'
_N='Click Here To Buy'
_M='setting'
_L='type the text and send! ✍️\n\n/skip the Text'
_K='text'
_J='postid'
_I='video_id'
_H='photo_id'
...
async def start(update,context):
	...
	else:B=[[InlineKeyboardButton(_N,_O)]];C=InlineKeyboardMarkup(B);await A.message.reply_text('To Buy BOT Contact : @r5pro\u200c\u200c',reply_markup=C)
async def buy(update,context):'Starts the post bot.';A=[[InlineKeyboardButton(_N,_O)]];B=InlineKeyboardMarkup(A);await update.message.reply_text('contact @r5pro\u200c\u200c',reply_markup=B)
async def newpost(update,context):
	A=update
	if str(A.effective_user.id)==str(owner):context.user_data[_C]=[];await A.message.reply_text(_T);return PHOTO
...
async def photo(update,context):'Stores the photo and asks for a location.';A=update;context.user_data[_H]=A.message.photo[-1].file_id;await A.message.reply_text(_L);return MESS
async def video(update,context):
	...
	else:context.user_data[_I]=A.message.video.file_id;await A.message.reply_text(_L);return MESS
async def edit_video(update,context):
	'Stores the photo and asks for a location.';A=update;B=f"video file size should be less then 20mb";C=23000000
	if A.message.video.file_size>C:await A.message.reply_text(B)
	else:DATALIST[context.user_data[_J]][_I]=A.message.video.file_id;await A.message.reply_text(_L);return MESS
async def edit_photo(update,context):
	...
	else:DATALIST[context.user_data[_J]][_H]=A.message.photo[-1].file_id;await A.message.reply_text(_L);return MESS
async def save_post(update,context):
	'Stores the photo and asks for a location.';A=update;B=A.message.text
	if B in DATALIST:context.user_data[_J]=B;await A.message.reply_text(_T);return PHOTO
	else:await A.message.reply_text('This post ID not avilable.\n Enter correct ID or press /cancel to cancel')
async def skip_photo(update,context):'Skips the photo and asks for a message.';await update.message.reply_text(_L);return MESS
...
async def edit_mess(update,context):
	'Stores the location and asks for some info about the user.';A=update
	try:B=A.message.text;C=context.user_data[_J];DATALIST[C][_K]=B;await A.message.reply_text(A.message.text);D=[[_D,_E],[_F,_P]];await A.message.reply_text('Pease enter the name of link button :  \n\n/done',reply_markup=ReplyKeyboardMarkup(D,resize_keyboard=_A,one_time_keyboard=_A))
	except BaseException:await A.message.reply_text('it should be text send again');return MESS
	return NAME
...
async def link_name(update,context):'Stores the links';A=update;B=A.message.text;context.user_data[_Q]=B;await A.message.reply_text('Please enter the link \n\nlink should be valid');return LINK
async def link(update,context):
	'Stores the links';C=update;A=context;D=C.message.text;E=A.user_data[_Q]
	...
	H=[[_D,_E],[_F,_P]];await C.message.reply_text(_U,reply_markup=ReplyKeyboardMarkup(H,resize_keyboard=_A,one_time_keyboard=_A));return NAME
async def edit_link(update,context):
	'Stores the links';C=update;A=context;I=A.user_data[_J];D=C.message.text;E=A.user_data[_Q]
	try:G=A.user_data[_C];B=InlineKeyboardButton(E,D);G.append(B)
	except BaseException:F=[];B=InlineKeyboardButton(E,D);F.append(B);A.user_data[_C]=F
	H=[[_D,_E],[_F,_P]];await C.message.reply_text(_U,reply_markup=ReplyKeyboardMarkup(H,resize_keyboard=_A,one_time_keyboard=_A));return NAME
async def skip_link(update,context):
	...
	except BaseException:await A.message.reply_text(_V)
	...
	M=[[_D,_E],[_F,_G]];await A.message.reply_markdown_v2(_W,reply_markup=ReplyKeyboardMarkup(M,resize_keyboard=_A,one_time_keyboard=_A,input_field_placeholder=_R));await A.message.reply_text(f"/send {B} (time in seconds)\n/time {B} (time in minutes) send single post");return ConversationHandler.END
...
async def edit_skip_link(update,context):
	...
	K=[[_D,_E],[_F,_G]];await A.message.reply_markdown_v2(_W,reply_markup=ReplyKeyboardMarkup(K,resize_keyboard=_A,one_time_keyboard=_A,input_field_placeholder=_R));await A.message.reply_text(f"/send {B} (time in seconds)\n/time {B} (time in minutes) send single post");return ConversationHandler.END
async def cancel(update,context):'Cancels and ends the conversation.';A=update;B=A.message.from_user;C=[[_D,_E],[_F,_G]];logger.info(_X,B.first_name);await A.message.reply_text('Bye!.',reply_markup=ReplyKeyboardMarkup(C,resize_keyboard=_A,one_time_keyboard=_A,input_field_placeholder=_R));return ConversationHandler.END
async def leftinmiddle(update,context):'Cancels and ends the conversation.';A=update;B=A.message.from_user;logger.info(_X,B.first_name);await A.message.reply_text('U HAVE LEFT in middle of post press /cancel')
async def send(update,context):
	'start the send';I='True';G=context;A=update;J=A.message.from_user;logger.info(_Y,J.first_name);await A.message.reply_text(nonono)
	if str(A.effective_user.id)==str(owner):
		B=G.args[0];K=float(G.args[1])
		try:D=DATALIST[B][_I]
		except BaseException:
			try:E=DATALIST[B][_H]
			except BaseException:pass
		...
		DATALIST[B][_S]=I;H=post_count;await A.message.reply_text(f"stop by /stop {B}")
		while DATALIST[B][_S]==I and H>0:
			if str(A.effective_user.id)==str(owner):
				try:await A.message.reply_video(video=D,caption=C,reply_markup=F,quote=_B)
				except BaseException:
					try:await A.message.reply_video(video=D,caption=C,quote=_B)
					except BaseException:
						try:await A.message.reply_video(video=D,quote=_B)
						except BaseException:
							try:await A.message.reply_photo(photo=E,caption=C,reply_markup=F,quote=_B)
							except BaseException:
								try:await A.message.reply_photo(photo=E,caption=C,quote=_B)
								except BaseException:
									try:await A.message.reply_photo(photo=E,quote=_B)
									except BaseException:
										try:await A.message.reply_text(C,reply_markup=F,quote=_B)
										except BaseException:await A.message.reply_text(C,quote=_B)
				H-=1;await sleep(K*waitpost)
			else:break
	else:L=[[InlineKeyboardButton(_N,_O)]];M=InlineKeyboardMarkup(L);await A.message.reply_text(_Z,reply_markup=M)
async def sendonce(update,context):
	'start the send';H=context;A=update;I=A.message.from_user;logger.info(_Y,I.first_name);await A.message.reply_text(nonono)
	if str(A.effective_user.id)==str(owner):
	...
	else:J=[[InlineKeyboardButton(_N,_O)]];K=InlineKeyboardMarkup(J);await A.message.reply_text(_Z,reply_markup=K)
async def stoppost(update,context):
	...
	if str(A.effective_user.id)==str(owner):
		try:C=context.args[0];DATALIST[C][_S]='false';await A.message.reply_text('stopped')
...
async def timeset1(update,context):
	...
	if isinstance(C,str):
		if C[0]=='h':waitpost=600;A=[[_D,_E],[_F,_G]];D=ReplyKeyboardMarkup(A);await B.message.reply_text('Time Format Changed TO Hours',reply_markup=ReplyKeyboardMarkup(A,resize_keyboard=_A,one_time_keyboard=_A,input_field_placeholder=_M));return ConversationHandler.END
		elif C[0]=='m':waitpost=60;A=[[_D,_E],[_F,_G]];D=ReplyKeyboardMarkup(A);await B.message.reply_text('Time Format Changed TO Minutes',reply_markup=ReplyKeyboardMarkup(A,resize_keyboard=_A,one_time_keyboard=_A,input_field_placeholder=_M));return ConversationHandler.END
		elif C[0]=='s':waitpost=1;A=[[_D,_E],[_F,_G]];D=ReplyKeyboardMarkup(A);await B.message.reply_text('Time Format Changed TO SEC',reply_markup=ReplyKeyboardMarkup(A,resize_keyboard=_A,one_time_keyboard=_A,input_field_placeholder=_M));return ConversationHandler.END
...
async def setting(update,context):'show two buttons for setting';A=update;C=A.message.from_user;logger.info('User %s pressed setting.',C.first_name);B=[['Post Count','Time Setting']];D=ReplyKeyboardMarkup(B);await A.message.reply_text('Settings for Bot!',reply_markup=ReplyKeyboardMarkup(B,resize_keyboard=_A,one_time_keyboard=_A,input_field_placeholder=_M));return RTPT
